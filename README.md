...
Para un binding que Star Citizen muestra como `AltGr + ;` debido a su compatibilidad limitada con el teclado español, escribe `AltGr+Ñ` en NOVA DECK. La app envía **Alt derecho + la posición física de Semicolon (scan code 0x27)**, que corresponde a la tecla Ñ del teclado español. También se acepta `AltRight+Semicolon`; no uses `Ctrl+Alt+Ñ` para este caso.
