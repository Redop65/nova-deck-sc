...
def test_parses_combo_and_aliases() -> None:
    ...
    physical_semicolon = ParsedKey("oem_semicolon", False, 0x27)
    assert parse_combo("AltGr+Ñ") == [ParsedKey("alt_gr", True), physical_semicolon]
    assert parse_combo("AltRight+Semicolon") == [ParsedKey("alt_gr", True), physical_semicolon]
